...
export const getBookById = async (req: Request, res: Response) => {
  ...
  if(isNaN(id) || id<=0) return res.status(400).json({message: "Invalid book ID format."});
  ...
  try{
    ...
    console.error(err);
    res.status(500).json({message: `Server failed to fetch book with id=${id}`});
  }
}

export const createBook = async (req: AuthRequest, res: Response) =>{
  try{
    const {title, author, description, isbn, price, stockQuantity, images} = req.body;

    if(!title || !author || !isbn || price===undefined || stockQuantity === undefined){
    ...
    if(existingBook) return res.status(409).json({message: "A book with this ISBN already exists."});
    ...
    console.error(err);
    res.status(500).json({message: "Server error while attempting to create book."});
  }
};
...
export const updateBook = async (req: AuthRequest, res: Response) => {
  ...
  try{
    ...
    if(!bookToUpdate) return res.status(404).json({message: `Book with ID ${id} not found`});

    if(bookToUpdate.isbn != isbn){
      const existingBook = await bookRepo.findOneBy({isbn});
      if(existingBook) return res.status(409).json({ message: "The new ISBN is already in use by another book." });
    }
    ...
    res.status(500).json({ message: `Server error while attempting to update book with id=${id}.` });
  }
}
...
export const deleteBook = async (req: AuthRequest, res: Response) => {
  ...
  try{
    ...
    if(!bookToDelete) return res.status(404).json({message: `Book with ID ${id} not found`});
